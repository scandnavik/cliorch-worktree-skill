#!/bin/bash
# wt-merge.sh — Sequentially merge done worktree branches with HITL on conflict
#
# Usage: bash wt-merge.sh <project-dir> [--skip-failed]
...
# Read tasks, sort by dependsOn, filter to done
BRANCHES=$(node -e "
